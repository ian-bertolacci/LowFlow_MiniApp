...
void science(
  ...
  const int num_compute_kernels = 1 /*main kernels*/ + 1 /*reduce kernels*/;
  ...
  // Fusion of 261 338 416 and non-reduce portion of 551 analogue
  kernel_idx = 0;
  arg_idx = 0;
  // Setup arguments
  kernel_num_args[kernel_idx] = 23*2; /* 23 array-domain pairs */
  ...
  kernel_names[kernel_idx] = (char*)"function_NFE261_NFE338_NFE416_NFE551";
  compute_kernel_sources[kernel_idx] = (char*) STRINGIZE(
    __kernel void function_NFE261_NFE338_NFE416_NFE551(
      __global   double*         fp,
  ...
  double* timers[num_compute_kernels] = { &metrics->elapsed_216_338_416_551, &metrics->elapsed_551_reduce };
  #endif
  // Invoke kernels
  for( int k = 0; k < num_compute_kernels; k += 1 ){
